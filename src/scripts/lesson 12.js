(function(){
	let canvas = document.getElementById('canvas_12'),
	...
	Rect.prototype = {
		...
		gravity: function(){
			...
			// Если мы в воде
			if(this.y + this.h/1.5 > height - 150){
				this.maxSpeed = 2;
				this.acselerate = 0.1;

				// Если мы только коснулись воды
				if(this.dy > this.maxSpeed && this.y < height - 150){
					this.dy = this.dy / this.maxSpeed;
				}
			}
			...
			if(this.y + this.h >= height){
				...
				// this.dy /= 2; // Значение упругости (насколько сильно будет отскакивать)
	...
	setInterval(function(){
		...
		context.fillStyle = '#529CFD';
		drawFillRect(0, height - 150, width, height);
		context.fillStyle = '#2F61FD';
		drawFillRect(0, height - 100, width, height);
		context.fillStyle = '#3823CA';
		drawFillRect(0, height - 50, width, height);
